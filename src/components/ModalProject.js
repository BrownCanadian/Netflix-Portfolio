const ModalProject = ({open, img , genre , cast , desc, name, openFunction})=>{
    ...
    if(!open)return null;
    return(
        <div onClick={()=>{openFunction(false)}} className="w-screen h-screen bg-transparent backdrop-blur-sm rounded-3xl fixed top-0 z-100 flex flex-col justify-center">
            <div onClick={(e)=>e.stopPropagation()} className="sm:hideen md:block mx-auto md-w-2/3 lg:w-2/5 xl:w-2/5 rounded- bg-red-300 flex flex-col">
                <div className="w-full flex flex-col">
                    ...
                    <h1 className="absolute md:mt-56 lg:mt-40 xl:mt- h-1/3  text-white font-netflix text-5xl ml-8 bg-transparent drop-shadow-[0_1.2px_1.2px_rgba(0,0,0,0.8)] z-20">{name}</h1>
                ...
                <div className="flex px-6 ">
                    ...
                    <div className="flex flex-col justify-start w-1/3">
                        <div className="flex flex-row flex-wrap">
                            <p className="text-white font-netflix">Cast:</p>
                            {cast.map(c => <span className="text-white font-netflix mx-1">{c}</span> )}
                        </div>
                        <div className="flex flex-row flex-wrap pt-4">
                            <p className="text-white font-netflix ">Genre:</p>
                            {genre.map(g => <span className="text-white font-netflix mx-1">{g}</span> )}
                        </div>
                    </div>
                </div>
            </div>
